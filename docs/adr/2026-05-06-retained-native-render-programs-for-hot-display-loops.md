...
# ADR 2026-05-06: Add retained native render programs for hot display loops
...
Extend the existing v2 protocol with retained native render programs.

A retained native render program is a native-side object that is configured from Elixir, then executed repeatedly by native code. Elixir remains responsible for setup, resource creation, initial data upload, lifecycle control, and optional low-frequency updates. Native code owns the hot display loop.
...
The existing v2 protocol remains the immediate-call and binary-batch protocol. Retained native render programs are added as an extension for workloads where the display frame loop must run natively.
...
The initial render program type should support striped transformed-sprite rendering.
...
Elixir
  ...
  creates a retained render program
...
The first useful render program can be modeled as:

```text
striped_sprite_transform
```
...
Retained native render programs are added without changing the protocol version.
...
A native render program may need exclusive display ownership while running.
...
```elixir
AtomLGFX.RenderProgram.start(program, mode: :exclusive)
```
...
{:ok, object_buffer} =
  AtomLGFX.create_object_buffer(display,
    layout: :sprite_transform_2d,
    capacity: 50
  )

:ok = AtomLGFX.write_object_buffer(object_buffer, objects)

{:ok, program} =
  AtomLGFX.RenderProgram.create(display,
    type: :striped_sprite_transform,
    object_buffer: object_buffer,
    sources: [icon0, icon1, icon2],
    strip_height: 160,
    background_color: 0,
    transparent_color: 0,
    update: :bounce,
...
:ok = AtomLGFX.RenderProgram.start(program, mode: :exclusive)

{:ok, stats} = AtomLGFX.RenderProgram.stats(program)

:ok = AtomLGFX.RenderProgram.stop(program)
```
...
Retained native render programs answer:
...
The retained render-program design keeps the native hot loop while avoiding a demo-specific API.
...
- additional render-program types
